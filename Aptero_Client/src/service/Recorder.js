...
class Recorder {
    ...
    ispause: boolean;                   // 是否是暂停
    context: any;
    config: recorderConfig;             // 配置
    size: number;                       // 录音文件总长度
    lBuffer: Array<Float32Array> = [];  // pcm音频数据搜集器(左声道)
    rBuffer: Array<Float32Array> = [];  // pcm音频数据搜集器(右声道)
    PCM: any;                           // 最终的PCM数据缓存，避免多次encode
    tempPCM: Array<DataView> = [];      // 边录边转时临时存放pcm的
    audioInput: any;
    inputSampleRate: number;            // 输入采样率
    source: any;                        // 音频输入
    recorder: any;
    inputSampleBits: number = 16;       // 输入采样位数
    outputSampleRate: number;           // 输出采样率
    oututSampleBits: number;            // 输出采样位数
    analyser: any;
    littleEdian: boolean;               // 是否是小端字节序
    prevDomainData: any;                // 存放前一次图形化的数据
    playStamp: number = 0;              // 播放录音时 AudioContext 记录的时间戳
    playTime: number = 0;               // 记录录音播放时长
